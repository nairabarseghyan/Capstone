...
FIGURES_PATH <- "/Users/nairabarseghyan/Desktop/project_name/reports/LaTex/figures/"
DATA_PATH <- "/Users/nairabarseghyan/Desktop/project_name/data/"
```

# Importing preprocessing functions
```{r Preprocessing-Script, include=FALSE}
...
```{r Add-Calendar-Data, include=FALSE}
calendar_sales_data <- add_calendar_features(sales_data_preprocessed)
```

```{r Get-Final-Data, include=FALSE}
final_sales_data <- add_sales_proxies(calendar_sales_data)
```

```{r General-Revenue-Dataframe}
aggregated_revnue_dataframe<-general_revenue_dataframe(final_sales_data)
```

# Feuture engenereeing 

```{r Factor-Categorcal-Data, include=FALSE}
...
```{r Anova-Dataframe, include=FALSE}
...
```{r Save-Anova, include=FALSE}
write.csv(format(anova_dataframe, digits=2), paste0(DATA_PATH, "results/Models/anova_results.csv"), row.names = FALSE)
```

```{r Print-Anova, echo=FALSE, message=FALSE}
...
# Helper functions
```{r Transform-Labels-To-Millions, include=FALSE}
...
```{r File-Saving-Helper, include=FALSE}
# Function to get current device size
get_device_size <- function() {
  # Check if a device is open and return its size
  if (dev.cur() != 1) {
    return(dev.size("in"))  # Return size in inches
  } else {
    return(c(7, 5))  # Default size in inches if no device is open
  }
}
```
# EDA
...
data_for_correlation <- final_sales_data %>%
  select(Revenue_VAT, Quantity, sales_proxy_week, sales_proxy_month) %>%
  na.omit()  # Removing NA values to ensure correlation calculation
cor_matrix <- cor(data_for_correlation)
```



```{r Correlation-Plot ,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "correlation_matrix_with_style.pdf"), width = current_size[1], height = current_size[2])
plot_corr <- ggcorrplot(cor_matrix, 
                        lab = TRUE, 
                        lab_size = 3.5,
                        type = "lower", # Lower triangle of the matrix
                        hc.order = TRUE, # Hierarchical clustering to reorder the matrix
                        outline.col = "white", # White outlines for separation
                        colors = custom_color_palette(3), # Using the defined colors
                        tl.srt = 45) # Text rotation for tick labels
...
```{r Time-Series-Graphing, include=FALSE}
...
```{r Time-Series-2021,echo = FALSE, out.height='80%', message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "time_series_2021_sales.pdf"), width = current_size[1], height = current_size[2])
...
```{r Time-Series-2022,echo = FALSE , out.height='80%', message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "time_series_2022_sales.pdf"), width = current_size[1], height = current_size[2])
...
```{r Time-Series-2023,echo = FALSE, out.height='80%', message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "time_series_2023_sales.pdf"), width = current_size[1], height = current_size[2])
...
```{r Sale-By-Branch-Barplot-Graphing, include=FALSE}
plot_sales_by_branch<- function(df, target_year = NULL, target_city= NULL) {
  ...
  data <- df%>%filter(
          ...
          mutate(label = ifelse(Total > 500e6, label_millions(Total), NA))
...
```{r Sales-By-Shop-2021,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_by_shop_2021_sales.pdf"), width = current_size[1], height = current_size[2])
sales_by_shop_2021_sales<-plot_sales_by_branch(
  df = final_sales_data, 
  target_year = 2021
)
print(sales_by_shop_2021_sales)
...
```{r Sales-By-Shop-2022,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_by_shop_2022_sales.pdf"), width = current_size[1], height = current_size[2])
sales_by_shop_2022_sales<-plot_sales_by_branch(
  df = final_sales_data, 
  target_year = 2022
)
print(sales_by_shop_2022_sales)
grid::grid.raster(logo, x = 0.03, y = -0.04, just = c('left', 'bottom'), width = unit(1, 'inches'))
dev.off()
print(sales_by_shop_2022_sales)
grid::grid.raster(logo, x = 0.03, y = -0.04, just = c('left', 'bottom'), width = unit(1, 'inches'))
...
```{r Sales-By-Shop-2023,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_by_shop_2023_sales.pdf"), width = current_size[1], height = current_size[2])
sales_by_shop_2023_sales<-plot_sales_by_branch(
  df = final_sales_data, 
  target_year = 2023
)
print(sales_by_shop_2023_sales)
grid::grid.raster(logo, x = 0.03, y = -0.04, just = c('left', 'bottom'), width = unit(1, 'inches'))
dev.off()
print(sales_by_shop_2023_sales)
grid::grid.raster(logo, x = 0.03, y = -0.04, just = c('left', 'bottom'), width = unit(1, 'inches'))
...
```{r Sales-By-Shop-Yerevan,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_by_shop_yerevan_sales.pdf"), width = current_size[1], height = current_size[2])
sales_by_shop_yerevan_sales<-plot_sales_by_branch(
  df = final_sales_data, 
  target_city = "Yerevan"
)
print(sales_by_shop_yerevan_sales)
grid::grid.raster(logo, x = 0.03, y = -0.04, just = c('left', 'bottom'), width = unit(1, 'inches'))
dev.off()
print(sales_by_shop_yerevan_sales)
grid::grid.raster(logo, x = 0.03, y = -0.04, just = c('left', 'bottom'), width = unit(1, 'inches'))
...
```{r Heatmap-Sales-Year-Month,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "heatmap_sales.pdf"), width = current_size[1], height = current_size[2])
...
```{r, Area-Chart-Data, echo=FALSE, message=FALSE}
...
```{r Area-Chart-Graph ,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "area_graph_sales_by_year.pdf"), width = current_size[1], height = current_size[2])
p <- ggplot(df_surfacegraph, aes(x = month, y = Total_Revenue , fill = year, group = year)) +
  ...
  labs(title = "Total Sales by Month and Year", x = "", y = "Total Revenue", fill = "Year") +
  theme_minimal() +
...
```{r Revenue-By-City_data, include=FALSE}
...
```{r Revenue-City-Without-Yerevan,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_by_city.pdf"), width = current_size[1], height = current_size[2])
revenue_wiothot_yerevan<-city_revenue  %>%
    filter(City != "Yerevan") %>%
...
```{r Treemap-whole-save,include= FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH,"treemap_whole.pdf"), width = current_size[1], height = current_size[2])
...
```{r Treemap-whole-print,echo = FALSE, message=FALSE}
treemap(city_revenue,
  index = "City",          # The column that identifies each rectangle
  vSize = "Total_Revenue", # The column that determines the area size of each rectangle
  vColor = "Total_Revenue",# The column that determines the color scale
...
```{r Treemap-Without-Yerevan-save, include = FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH,"treemap_without_yerevan.pdf"), width = current_size[1], height = current_size[2])
city_revenue %>% 
  filter(City != "Yerevan") %>%
        treemap(
        index = "City",          # The column that identifies each rectangle
...
```{r Treemap-Without-Yerevan-print, echo = FALSE, message=FALSE}
city_revenue %>% 
  filter(City != "Yerevan") %>%
        treemap(
        index = "City",          # The column that identifies each rectangle
...
```{r Sales-Weekday,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_by_weekday.pdf"), width = current_size[1], height = current_size[2])
p <- ggplot(data_summary, aes(x = day_name, y = Total_Revenue, fill = City)) +
  ...
  labs(title = "Average Sales by Weekday Across Cities", x = "", y = "Revenue in Millions") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
```{r Category-Sales-Number-Data, include=FALSE}
...
```{r Category-Sales-Number-Graph,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_qunatity_by_category.pdf"), width = current_size[1], height = current_size[2])
...
```{r Category-Revenue-Data, include=FALSE}
...
```{r Category-Revenue-Graph,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "revenue_by_category.pdf"), width = current_size[1], height = current_size[2])
...
```{r Category-Revenue-Quantitiy-Graph,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "revenue_quantity_comparison_by_category.pdf"), width = current_size[1], height = current_size[2])
...
```{r, Top-Brand-Data, include=FALSE}
...
```{r Top-10-Brands,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "top_10_brands.pdf"), width = current_size[1], height = current_size[2])
...
```{r Top-Heating-Tiles-Data, include=FALSE}
...
Tiles_data <- final_sales_data %>%
  filter(Product_category == "Tiles",Brand != "",) %>%
  group_by(Brand) %>%
  summarise(Total_rev = sum(Revenue_VAT, na.rm = TRUE)) %>%
  top_n(5, Total_rev) %>%
  ungroup() %>%
...
```{r Top-Heating-Graph, echo=FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "top_5_brands_heating.pdf"), width = current_size[1], height = current_size[2])
...
```{r Top-Tiles-Graph,echo = FALSE, message=FALSE}
...
```{r Top-Heating-Tiles-Graph,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "top_5_tiles_heating.pdf"), width = current_size[1], height = current_size[2])
...
```{r Load-Armenia-Map,include=FALSE}
...
```{r Armenia-Map-Graph,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_by_map_armenia.pdf"), width = current_size[1], height = current_size[2])
...
```{r Load-Yerevan-Map, include=FALSE}
...
```{r Yerevan-Map-Graph,echo = FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "sales_by_map_yerevan.pdf"), width = current_size[1], height = current_size[2])
...
```{r Load-Prediction-Data, include=FALSE}
# Load One Layer LSTM predictions Data
LSTM_ONE <- read.csv(paste0(DATA_PATH, "results/Predictions/LSTM_one_predictions.csv"))

# Load Two Layer LSTM predictions Data
LSTM_TWO <- read.csv(paste0(DATA_PATH, "results/Predictions/LSTM_multi_predictions.csv"))

# Load Zero-Shot Lag-Llama predictions Data
ZER_SHOT_LLAMA <- read.csv(paste0(DATA_PATH, "results/Predictions/lag_llama_zero_shot.csv"))

# Load Fine-Tuned Lag-Llama predictions Data
FINE_TUNED_LLAMA <- read.csv(paste0(DATA_PATH, "results/Predictions/lag_llama_fine_tune.csv"))

# Load Random Forest Regressor Data
#Train
RFR_train <- read.csv(paste0(DATA_PATH, "results/Predictions/RFR_train_predictions.csv"))

#Validation
RFR_val <- read.csv(paste0(DATA_PATH, "results/Predictions/RFR_val_predictions.csv"))

#Test
RFR_test <- read.csv(paste0(DATA_PATH, "results/Predictions/RFR_test_predictions.csv"))
```


```{r Transform-Date-Col, include=FALSE}
# One Layer LSTM predictions Data
LSTM_ONE$X <- as.POSIXct(LSTM_ONE$X, format = "%Y-%m-%d")

# Two Layer LSTM predictions Data
LSTM_TWO$X <- as.POSIXct(LSTM_TWO$X, format = "%Y-%m-%d")

#  Zero-Shot Lag-Llama predictions Data
ZER_SHOT_LLAMA$date <- as.POSIXct(ZER_SHOT_LLAMA$date, format = "%Y-%m-%d")

#  Fine-Tuned Lag-Llama predictions Data
FINE_TUNED_LLAMA$date <-as.POSIXct(FINE_TUNED_LLAMA$date, format = "%Y-%m-%d")

#  Random Forest Regressor Data
#Train
RFR_train$Date <- as.POSIXct(RFR_train$Date, format = "%Y-%m-%d")
RFR_val$Date <- as.POSIXct(RFR_val$Date, format = "%Y-%m-%d")
RFR_test$Date <- as.POSIXct(RFR_test$Date, format = "%Y-%m-%d")
```

```{r LSTM-One-Forecast, echo=FALSE, out.height='80%', message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "one_layer_lstm_prediction.pdf"), width = current_size[1], height = current_size[2])

lstm_one_forecast<- ggplot() +
  
  geom_line(data = slice(aggregated_revnue_dataframe,811: 1051), aes(x = sale_date, y = Revenue), color = start_color, size = 1) +
  
  geom_line(data = tail(LSTM_ONE, 240), aes(x = X, y = Prediction), color = end_color, size = 1, alpha = 1) +
  scale_x_datetime(date_labels = '%b', breaks = scales::date_breaks("1 month")) +
  scale_y_continuous(labels = label_millions) +
  labs(x = "", y = "Revenue", title = "One Layer LSTM Forecast") +
  theme_minimal() +
  theme(text = element_text(size = 15),  # Update font size
        axis.text.x = element_text(hjust = 1))  # Rotate x axis labels
print(lstm_one_forecast)
dev.off()
print(lstm_one_forecast)
```


```{r LSTM-Two-Forecast, echo=FALSE, out.height='80%', message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "two_layer_lstm_prediction.pdf"), width = current_size[1], height = current_size[2])


two_layer_lstm_prediction<-ggplot() +
  
  geom_line(data = slice(aggregated_revnue_dataframe,811: 1051), aes(x = sale_date, y = Revenue), color = start_color, size = 1) +
  
  geom_line(data = tail(LSTM_TWO, 240), aes(x = X, y = Prediction), color = end_color, size = 1, alpha = 1) +
  scale_x_datetime(date_labels = '%b', breaks = scales::date_breaks("1 month")) +
  scale_y_continuous(labels = label_millions) +
  labs(x = "", y = "Revenue", title = "Two Layer LSTM Forecast") +
  theme_minimal() +
  theme(text = element_text(size = 15),  # Update font size
        axis.text.x = element_text(hjust = 1))  # Rotate x axis labels
print(two_layer_lstm_prediction)
dev.off()

print(two_layer_lstm_prediction)
```
```{r LSTM-Predictions, echo=FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "lstm_predictions.pdf"), width = current_size[1], height = current_size[2])
ggarrange(lstm_one_forecast, two_layer_lstm_prediction, nrow = 2)
dev.off()
```

```{r Zero-Shot-Forecast, echo=FALSE, out.height='80%', message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "zero_shot_lag_llam_prediction.pdf"), width = current_size[1], height = current_size[2])


zero_shot_lag_llam_prediction<-ggplot() +
  # Plot the target data (assuming the target data is structured similarly to 'all_preds')
  geom_line(data = tail(aggregated_revnue_dataframe, 240), aes(x = sale_date, y = Revenue), color = start_color, size = 1) +
  # Plot the median forecast
  geom_line(data = ZER_SHOT_LLAMA, aes(x = date, y = pred), color = end_color, size = 1, alpha = 1, label = "Forecast") +
  # Plot prediction intervals
  geom_ribbon(data = ZER_SHOT_LLAMA, aes(x = date, ymin = p10, ymax = p90), fill = end_color, alpha = 0.2, inherit.aes = FALSE) +
  scale_x_datetime(date_labels = '%b', breaks = scales::date_breaks("1 month")) +
  scale_y_continuous(labels = label_millions) +
  labs(x = "", y = "Revenue", title = "Zero-Shot, Lag-Llama Forecast") +
  theme_minimal() +
  theme(text = element_text(size = 15),  # Update font size
        axis.text.x = element_text(hjust = 1))  # Rotate x axis labels

print(zero_shot_lag_llam_prediction)
dev.off()
...
```{r Fine-Tune-Forecast, echo=FALSE, out.height='80%', message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "fine_tune_lag_llam_prediction.pdf"), width = current_size[1], height = current_size[2])


fine_tune_lag_llam_prediction<-ggplot() +
  # Plot the target data (assuming the target data is structured similarly to 'all_preds')
  geom_line(data = tail(aggregated_revnue_dataframe, 240), aes(x = sale_date, y = Revenue), color = start_color, size = 1) +
  # Plot the median forecast
  geom_line(data = FINE_TUNED_LLAMA, aes(x = date, y = pred), color = end_color, size = 1, alpha = 1, label = "Forecast") +
  # Plot prediction intervals
  geom_ribbon(data = FINE_TUNED_LLAMA, aes(x = date, ymin = p10, ymax = p90), fill = end_color, alpha = 0.2, inherit.aes = FALSE) +
  scale_x_datetime(date_labels = '%b', breaks = scales::date_breaks("1 month")) +
  scale_y_continuous(labels = label_millions) +
  labs(x = "", y = "Revenue", title = "Fine-Tuned, Lag-Llama Forecast") +
  theme_minimal() +
  theme(text = element_text(size = 15),  # Update font size
        axis.text.x = element_text(hjust = 1))  # Rotate x axis labels

print(fine_tune_lag_llam_prediction)
dev.off()

print(fine_tune_lag_llam_prediction)
```
```{r Lag-Lama-Prediction, echo=FALSE, message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "lag_llam_prediction.pdf"), width = current_size[1], height = current_size[2])
ggarrange(zero_shot_lag_llam_prediction, fine_tune_lag_llam_prediction, nrow = 2)
dev.off()
```


```{r ML-Forecast-Graphing-Function, include=FALSE}


plot_sales_data <- function(stores, data, label) {


  # Aggregate sales and predictions by date
  Aggregate_data <- data %>%
    filter(Branch == stores) %>%
    group_by(Date) %>%
    summarise(Revenue_VAT = sum(True_value), Pred = sum(Pred), .groups = "drop")


  # Plotting setup
  p <- ggplot(data = Aggregate_data) +
    geom_line(aes(x = Date, y = Revenue_VAT), color = start_color) +
    geom_line(aes(x = Date, y = Pred), color = end_color) +
    scale_x_datetime(date_labels = '%b', breaks = scales::date_breaks("1 month")) +
  scale_y_continuous(labels = label_millions) +
    theme_minimal() +
    labs(title = label,subtitle = stores, x = "", y = "Revenue") 
...
```{r ML-Forecast-Davit-Anhaght, echo=FALSE, out.height='80%', message=FALSE}
current_size <- get_device_size()
pdf(paste0(FIGURES_PATH, "ML_davit_anhaght_prediction.pdf"), width = current_size[1], height = current_size[2])

ML_davit_anhaght_prediction<-plot_sales_data('Davit Anhaght S.',RFR_val, "Random Forest Regressor Forecast")
dev.off()

print(ML_davit_anhaght_prediction)
```

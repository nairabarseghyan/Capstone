...
```{r}
logo <- image_read("./img/nt_logo.png")
```

# Reading and creating necessary datasets 

```{r}
sales_raw <- read.csv("data/translated_data/csv/sales_whole.csv")
holidays <- read.csv("data/translated_data/csv/armenian_holidays.csv")
```
# Preprocessing sales data

```{r}
sales_data_preprocessed <- process_sales_data(sales_raw, holidays)
```

# adding feutures to the data
```{r}
final_sales_data <- add_calendar_features(sales_data_preprocessed)
```
...
```{r}
write.csv(anova_dataframe, "./anova_results.csv", row.names = FALSE)
```
...
column_info
write.csv(column_info, "./attributes_info.csv", row.names = FALSE)
```
...
```{r}
create_sales_time_series <- function(df, target_year, annotations, logo_path= "./img/nt_logo.png", custom_subtitle = NULL) {
...
# annotations = list(
#   list(date = "2021-02-27", value = 92800000, label = "92.8M"),
#   list(date = "2021-03-15", value = 85000000, label = "85.0M")
# )
# create_sales_time_series(df = your_dataframe, target_year = 2021, annotations = annotations, logo_path = "./path/to/logo.png
...
create_sales_time_series(df = final_sales_data, 
                         target_year = 2021, 
                         annotations = annotations,
                          logo_path = "./img/nt_logo.png")
...
```{r}
plot_sales_by_branch<- function(df, target_year = NULL, target_city= NULL, logo_path = "./img/nt_logo.png") {
  ...
  if (!is.null(logo_path)) {
    logo <- png::readPNG(logo_path)
    grid::grid.raster(logo, x = 0.075, y = 0.001, just = c('left', 'bottom'), width = unit(1, 'inches'))
  }
   }
```
...
# PRobabaly will remove this graph not so good
lets try treemap 

```{r}
p <- ggplot(city_revenue, aes(x = City_Group, y = Total_Revenue, fill = City)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = 'Total Revenue for Yerevan and Other Cities', x = '', y = 'Total Revenue') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.x = element_blank(),
        plot.title = element_text(size = 16, face = "bold", hjust = 0.5)) +
  scale_fill_manual(values = custom_color_palette(12))
...
# Include the logo as previously described
logo_file <- "./img/nt_logo.png"  # Path to your logo file
logo <- png::readPNG(logo_file)
...
```{r}
#arm <- readOGR('Data/Armenia_Marzes')
#class(arm)
```
...
# # edit from here
# y <- "Revenue_VAT"
# cols <- 4:8 # the 4 continuous dependent variables
...
#   purrr::pmap(
#     .l = list(
#       data = list(as_tibble(final_sales_data)),
#       x = as.list(colnames(final_sales_data)[cols]),
#       y = y,
